...
ERROR_TITLE = "<:verify_red:1494668360049492150> Erreur"
...
def get_verify_channel_id(guild_id):
    config = load_config()
    return config.get(str(guild_id), {}).get("verify_channel_id", 0)


def set_verify_channel(guild_id: int, channel: discord.TextChannel):
    config = load_config()
    guild_key = str(guild_id)
    if guild_key not in config:
        config[guild_key] = {}
    config[guild_key]["verify_channel_id"] = channel.id
    save_config(config)
...
async def ask_gemini_about_bot(prompt: str, bot) -> str:
    ...
    guarded_prompt = (
        ...
        "Jai uniquement accès à 5 emojis : croix rouge : <:verify_red:1494668360049492150>, check bleu : <:verify:1494667945505456159>, lien bleu : <:link:1495499715620372510>, plus bleu : <:plus:1495495582872244275>, Je les utilise uniquement pour illustrer mes réponses quand c'est pertinent ",
