...
from .definitions import (
    ERROR_TITLE, get_verified_role, get_venus_channel_id, get_verified_role_id,
...
class VerifierCog(commands.Cog):
    ...
    async def verifier(self, interaction: discord.Interaction):
        ...
        if member and verified_role and verified_role in member.roles:
            embed = discord.Embed(
                title="<:venus:1494667945505456159> Déjà vérifié",
        ...
        channel_id = get_venus_channel_id(interaction.guild.id)
        if channel_id:
